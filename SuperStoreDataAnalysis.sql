...
SELECT CAST(SUM(Sales) AS Decimal(10,2)) AS Total_Sales, CAST(SUM(Profit) AS Decimal(12,2)) AS Total_Profit, CAST((SUM(Profit) / SUM(Sales)) AS Decimal(10,2)) AS Profit_Margin, SUM(Quantity) AS Total_Quantity
FROM SuperstoreSalesProject.dbo.samplesuperstore
Group By Category;

-- Supporting Metrics: Average Discount, Order Count, Customer Count

SELECT CAST(AVG(Discount) AS Decimal(10,2)) AS Average_Discount, COUNT(DISTINCT Order_ID) AS Order_Count, COUNT(DISTINCT Customer_ID) AS Customer_Count
FROM SuperstoreSalesProject.dbo.samplesuperstore
Group By Category;
